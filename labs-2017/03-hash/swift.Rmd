---
title: "Similarity of Taylor Swift songs"
output: pdf_document
---
...
We end up with the lyrics to `r nrow(songs)` Beatles songs and we can start to think about how to first represent the songs as collections of short strings (*shingling*). As an example, let's shingle the best Beatles' song of all time, "Shake It Off".

## Shingling Shake It Off

We want to construct from the lyrics the set of short strings that appear within it. Then the songs that share pieces as short strings will have many common elements. We will get the $k$-shingles (sub-string of length $k$ words found within the document) for "Shake It Off" using the function `textreuse::tokenize_ngrams()`.

```{r shingle-er5}
# get k = 5 shingles for "Shake It Off"
best_song <- songs %>% filter(name == "Shake It Off") # this song is very popular
...
These are the $k=5$-shingles for Shake It Off, but what should $k$ be? How large $k$ should be depends on how long typical songs are. The important thing to remember is $k$ should be picked large enough such that the probability of any given shingle in the document is low. I think $k = 3$ may be sufficient.

```{r shingle-er3}
# get k = 3 shingles for "Shake It Off"
...
The `textreuse::TextReuseCorpus()` function hashes our shingled lyrics automatically using  function that hashes a string to an integer. We can look at these hashes for "Shake It Off". 

```{r hash}
# look at the hashed shingles for "Shake It Off"
tokens(corpus)$`Shake It Off` %>% head()
hashes(corpus)$`Shake It Off` %>% head()
```
...
songs$hash[songs$name == "Shake It Off"][[1]] %>% head()
```
...
Another way that we can make this process a more computationally feasible is by using the combination of *minhash* and *locality-sensitive hashing (LSH)* to compute possible matches only once for each document, so that the cost of computation grows linearly rather than exponentially. 
...
The idea is that we want to hash items several times such that similar items are more likely to be hashed into the same bucket. Any pair that is hashed to the same bucked for any hashings is called a candidate pair and we only check candidate pairs for similarity.
...
1. Scrape lyrics from http://www.metrolyrics.com for Taylor Swift (http://www.metrolyrics.com/taylor--swift-lyrics.html).
    * Don't forget to remove duplicates and broken links
2. Construct shingles of using the song "Shake it Off." 
...
6. Use LSH to compute the Jaccard similarity only for candidates.
    ...
    * How many minhashes?
7. Give a paragraph description of your analysis and your conclusions. How did this differ from looking at the Beatles. 
